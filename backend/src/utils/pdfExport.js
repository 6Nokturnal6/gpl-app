...
function buildPdfBuffer(data) {
  return new Promise((resolve, reject) => {
    ...
    function checkPage(px) {
      if (doc.y > doc.page.height - 60 - (px||30)) { doc.addPage(); drawHeader(); }
    }
    ...
    function tableHeader(cols) {
      ...
      doc.y = y + 16; doc.fillColor(DARK).font('Helvetica');
    }
    ...
    function finRow(label, value, isTotal) {
      ...
      doc.y = y + h + 1; doc.font('Helvetica');
    }
    ...
    tableHeader(docCols);
    (data.docentes||[]).forEach((r,i)=>{const tot=(parseInt(r.lic_h)||0)+(parseInt(r.lic_m)||0)+(parseInt(r.mest_h)||0)+(parseInt(r.mest_m)||0)+(parseInt(r.dout_h)||0)+(parseInt(r.dout_m)||0);tableRow(docCols,[r.regime==='tempo_inteiro'?'T.Inteiro':'T.Parcial',r.provincia,r.nacionalidade,r.lic_h||0,r.lic_m||0,r.mest_h||0,r.mest_m||0,r.dout_h||0,r.dout_m||0,tot],i%2===1);});
    ...
    tableHeader(invCols);
    (data.investigadores||[]).forEach((r,i)=>{const tot=(parseInt(r.lic_h)||0)+(parseInt(r.lic_m)||0)+(parseInt(r.mest_h)||0)+(parseInt(r.mest_m)||0)+(parseInt(r.dout_h)||0)+(parseInt(r.dout_m)||0);tableRow(invCols,[r.regime==='tempo_inteiro'?'T.Inteiro':'T.Parcial',r.nacionalidade,r.lic_h||0,r.lic_m||0,r.mest_h||0,r.mest_m||0,r.dout_h||0,r.dout_m||0,tot],i%2===1);});

    // Financas
    newSection('Recursos Financeiros — ' + YEAR);
    const fin=data.financas||{};
    subTitle('Quadro 2 – Financiamento por fonte');
    finRow('OGE (Orçamento Geral do Estado)',fin.oge);finRow('Doações (internas e externas)',fin.doacoes);finRow('Créditos',fin.creditos);finRow('Receitas próprias',fin.proprias);
    ...
    doc.moveDown(0.8);subTitle('Quadro 3 – Despesas correntes');
    finRow('Ensino',fin.func_ensino);finRow('Investigação',fin.func_investig);finRow('Administração',fin.func_admin);finRow('Salários – Docentes',fin.sal_docentes);finRow('Salários – Técnicos Administrativos',fin.sal_tecnicos);
    ...
    [['Ensino',summary.financas.func_ensino],['Investigação',summary.financas.func_investig],['Administração',summary.financas.func_admin],['Sal. Docentes',summary.financas.sal_docentes],['Sal. Técnicos',summary.financas.sal_tecnicos]].forEach(([l,v],i)=>tableRow(finSumCols,[l,(parseFloat(v)||0).toLocaleString('pt-MZ')],i%2===1));
