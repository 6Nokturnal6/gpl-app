...
function buildPdfBuffer(data) {
  return new Promise((resolve, reject) => {
    ...
    // ✅ NEW: safe footer handling (no re-render)
    let pageCount = 0;

    function drawFooter() {
      doc.save();
      doc.fillColor('#999').fontSize(7).font('Helvetica')
        .text(APP + '  ·  Página ' + pageCount, 40, doc.page.height - 20, { width: W, align: 'center' });
      doc.restore();
    }
    ...
    [['NUIT', d.nuit], ['Ano de início', d.ano_inicio], ['Província', d.provincia],
     ['Distrito', d.distrito], ['Website', d.website],
     ['Contacto', d.contacto], ['Email', d.email]]
    ...
    doc.fillColor(MID).fontSize(9).font('Helvetica-Bold')
      .text('Responsável pelo preenchimento');
