...
function buildPdf(data, res) {
  ...
  function checkPage(neededPx) {
    if (doc.y > doc.page.height - 60 - (neededPx || 30)) {
      doc.addPage();
      drawHeader();
    }
  }
  ...
  function tableRow(cols, values, shade) {
    ...
    cols.forEach((col, i) => {
      ...
      doc.fillColor(col.total ? BLUE : DARK).fontSize(7.5)
        ...
        .text(val, x + 2, y + 3, { width: col.w - 4, align: col.align || 'left' });
      x += col.w;
    });
    doc.y = y + 14;
  }
  ...
  [['OGE',summary.financas.oge],['Doações',summary.financas.doacoes],['Créditos',summary.financas.creditos],['Rec. próprias',summary.financas.proprias]]
    .forEach(([l,v],i)=>tableRow(finSumCols,[l,(parseFloat(v)||0).toLocaleString('pt-MZ')],i%2===1));
