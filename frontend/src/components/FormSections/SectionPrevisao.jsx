...
function emptyPrevisao() {
  return { curso:'', duracao:'', area:'', grau:'Licenciatura', provincia:'', homens:0, mulheres:0 };
}
...
export default function SectionPrevisao({ data, update }) {
  ...
  return (
    <Card title={`Previsão / Preliminar para ${NEXT_YEAR}`} desc={`Estudantes previstos por curso, grau e género (estimativa até novembro ${CURRENT_YEAR})`}>
      <TableWrap>
        <thead><tr><Th>Nome do curso</Th><Th>Dur.</Th><Th>Área</Th><Th>Grau</Th><Th>Província</Th><Th center>Homens</Th><Th center>Mulheres</Th><Th center>Total</Th><Th></Th></tr></thead>
        <tbody>
          {rows.map((r, i) => {
            ...
            return (
              <tr key={i} style={{ background: i%2===0?'transparent':'var(--color-background-secondary)' }}>
                ...
                <td style={td}><select value={r.grau||'Licenciatura'} onChange={e=>set(i,'grau',e.target.value)} style={{border:'none',background:'transparent',fontSize:12}}>{GRAUS.map(g=><option key={g}>{g}</option>)}</select></td>
        ...
        <tfoot>
          <tr>
            <td colSpan={5} style={{...td,textAlign:'right',fontWeight:500,fontSize:12,background:'var(--color-background-secondary)'}}>Total previsto {NEXT_YEAR}</td>
